...
BOT_TOKEN = "8520222825:8804348618:AAGChnE_w9jusklKSj9UmotvE8cnxH2BYjk"
...
# Bot endi kutilmagan (tarmoq, timeout va h.k.) xatolarda
# butunlay yiqilib qolmaydi - faqat logga yozadi va davom etadi.
# ============================================================
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Callback javobini yuborish (eskirgan/tarmoq xatolarini e'tiborsiz qoldiramiz)
    try:
        await query.answer()
    ...
    # Asosiy amalning o'zini ham xavfsizlantiramiz - shu yerda xato bo'lsa ham
    # bot yiqilib qolmaydi, faqat log yoziladi.
    try:
...
# ============================================================
# 📦 OMBOR HOLATI - BARCHA MAHSULOTLAR
# ============================================================
async def show_inventory(query):
    try:
        ...
        # Agar matn juda uzun bo'lsa (4000 belgidan oshsa), qismlarga bo'lamiz
        if len(text) > 4000:
            # Qismlarga bo'lish
            parts = []
            ...
            for p in products:
                ...
                line += f"   📊 {p['quantity']} {p['unit']}"
                if p['quantity'] <= p['min_quantity']:
                    line += f" (min: {p['min_quantity']})"
                line += f"\n   💰 {p['sell_price']:,.0f} so'm\n\n"
            ...
            # Birinchi qismni tahrirlab yuboramiz
            await query.edit_message_text(parts[0], parse_mode='Markdown')
            
            # Qolgan qismlarini yangi xabar sifatida yuboramiz
            for i in range(1, len(parts)):
                await query.message.reply_text(parts[i], parse_mode='Markdown')
            
            # Oxiriga bosh menyu tugmasini qo'shamiz
...
async def show_low_stock(query):
    try:
        ...
        text += "═" * 30 + "\n"
        text += f"📋 Jami: {len(products)} ta mahsulot kam qolgan"
        ...
        if len(text) > 4000:
            ...
            for p in products:
                line = f"🔴 *{p['name']}*\n"
                line += f"   📊 Qoldiq: {p['quantity']} {p['unit']}\n"
                line += f"   📉 Minimal: {p['min_quantity']}\n\n"
            ...
            current_part += "\n" + "═" * 30 + "\n"
            current_part += f"📋 Jami: {len(products)} ta mahsulot kam qolgan"
            parts.append(current_part)
...
def run_bot():
    ...
    try:
        # ===== TIMEOUTNI OSHIRISH VA PROXY QO'SHISH =====
        from telegram.request import HTTPXRequest
        
        # Timeoutni oshiramiz (agar internet sekin bo'lsa)
        ...
        # Proxy (agar kerak bo'lsa - masalan VPN orqali)
        # request = HTTPXRequest(proxy="http://proxy-server:8080")
        ...
        # Global xato-ushlagich - bot endi kutilmagan xatoda yiqilmaydi
        ...
        # drop_pending_updates=True - eski/osilib qolgan ulanishlarni tozalaydi
        # (bir nechta getUpdates so'rovi to'qnashuvi - "Conflict" xatosiga ham foydali)
