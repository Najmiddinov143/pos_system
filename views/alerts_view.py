...
class AlertsView(QWidget):
        ...
        try:
            ...
            for debt in debt_alerts:
                ...
                if due_date:
                    try:
                        ...
                        if 0 <= days_left <= 7:
                            existing = self.notification_repo.db.execute_query(
                                "SELECT * FROM notifications WHERE title = ? AND message LIKE ?",
                                ('⚠️ Nasiya to\'lov muddati', f'%{debt.get("product_name", "")}%')
                            )
                            if not existing:
            ...
            for product in products:
                if product.get('quantity', 0) <= product.get('min_quantity', 5):
                    existing = self.notification_repo.db.execute_query(
                        "SELECT * FROM notifications WHERE title = ? AND message LIKE ?",
                        ('⚠️ Mahsulot kam qolgan', f'%{product.get("name", "")}%')
                    )
                    if not existing:
    ...
    def mark_all_as_read(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            result = self.notification_repo.db.execute_update(
    ...
    def delete_alert(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            result = self.notification_repo.db.execute_query(
                "DELETE FROM notifications WHERE id = ?", (alert_id,)
            )
            if result:
                QMessageBox.information(self, "Muvaffaqiyat", "✅ Bildirishnoma o'chirildi!")
                self.load_alerts()
            else:
                QMessageBox.warning(self, "Xatolik", "Bildirishnomani o'chirishda xatolik!")