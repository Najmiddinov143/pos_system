...
class SettingsView(QWidget):
    ...
    def delete_user(self):
        try:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                result = self.user_repo.db.execute_query(
    ...
    def change_admin_password(self):
        try:
            ...
            admin_user = self.user_repo.db.execute_query_one("SELECT * FROM users WHERE username = 'admin'")
            if not admin_user:
            ...
            result = self.user_repo.db.execute_update(
                "UPDATE users SET password_hash = ? WHERE username = 'admin'", (new_hash,)
            )
