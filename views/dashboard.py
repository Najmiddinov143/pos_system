...
# views/dashboard.py - API GA MOSLASHTIRILGAN
...
class Dashboard(QWidget):
    def __init__(self, user_data):
        super().__init__()
        # user_data dict: {"token": "...", "role": "admin", "username": "admin"}
        self.user_data = user_data
        self.username = user_data.get('username', 'User')
        self.role = user_data.get('role', 'cashier')
    ...
    def setup_ui(self):
        ...
        title = QLabel(f"📊 Dashboard - Xush kelibsiz, {self.username}!")
        title.setObjectName("titleLabel")
        title.setStyleSheet("font-size: 28px; font-weight: bold; color: white;")
        header_layout.addWidget(title)
        ...
        # Cashier yoki Admin uchun
        if self.role == 'cashier':
            stats_row1 = [
                ('📈', 'Bugungi savdo', 'today_sales', '0 so\'m'),
                ('💰', 'Jami xarajat', 'total_expense', '0 so\'m'),
                ('🎁', 'Bonus/Chegirma', 'bonus_total', '0 so\'m'),
                ('💎', 'Sof foyda', 'net_profit', '0 so\'m'),
            ]
            stats_row2 = [
                ('💵', 'Naxt savdo', 'cash_sales', '0 so\'m'),
                ('💳', 'Plastik savdo', 'card_sales', '0 so\'m'),
                ('📝', 'Nasiya savdo', 'debt_sales', '0 so\'m'),
            ]
        else:
            stats_row1 = [
                ('📦', 'Mahsulotlar soni', 'products_count', '0'),
                ('💰', 'Umumiy tannarx', 'total_cost', '0 so\'m'),
                ('💵', 'Umumiy sotuv qiymati', 'total_value', '0 so\'m'),
                ('📈', 'Bugungi savdo', 'today_sales', '0 so\'m'),
            ]
            stats_row2 = [
                ('💹', 'Bugungi foyda', 'today_profit', '0 so\'m'),
                ('💰', 'Jami xarajat', 'total_expense', '0 so\'m'),
                ('🎁', 'Bonus/Chegirma', 'bonus_total', '0 so\'m'),
                ('💎', 'Sof foyda', 'net_profit', '0 so\'m'),
            ]
            stats_row3 = [
                ...
                ('💳', 'Plastik savdo', 'card_sales', '0 so\'m'),
                ('📝', 'Nasiya savdo', 'debt_sales', '0 so\'m'),
                ('🏆', 'Jami foyda', 'total_profit', '0 so\'m'),
            ]
        
        self.cards = {}
        row = 0
        
        for col, (icon, title, key, default) in enumerate(stats_row1):
            card = self.create_stat_card(icon, title, default)
            self.stats_layout.addWidget(card, row, col)
            self.cards[key] = card
        
        row = 1
        for col, (icon, title, key, default) in enumerate(stats_row2):
            card = self.create_stat_card(icon, title, default)
            self.stats_layout.addWidget(card, row, col)
            self.cards[key] = card
        ...
        if self.role == 'admin':
            row = 2
            for col, (icon, title, key, default) in enumerate(stats_row3):
                card = self.create_stat_card(icon, title, default)
                self.stats_layout.addWidget(card, row, col)
                self.cards[key] = card
        
        # Balance row
        row = 3 if self.role == 'admin' else 2
        balance_row = self.create_balance_row()
        self.stats_layout.addWidget(balance_row, row, 0, 1, 4)
    ...
    def create_balance_row(self):
        ...
        card = self.create_stat_card("💰", "Bugungi qoldiq (naqd)", "0 so'm")
        card.setFixedHeight(120)
        self.cards['today_balance'] = card
        h_layout.addWidget(card, 1)
        
        card_balance_widget = self.create_stat_card("💳", "Plastik qoldiq", "0 so'm")
        card_balance_widget.setFixedHeight(120)
        self.cards['card_balance'] = card_balance_widget
        h_layout.addWidget(card_balance_widget, 1)
        
        # Income button (faqat admin uchun)
        if self.role == 'admin':
            income_btn = QPushButton("➕\nKirim\nqo'shish")
            income_btn.setFixedHeight(120)
            income_btn.setFixedWidth(140)
    ...
    def add_income(self):
        """Kirim qo'shish (faqat admin uchun)"""
        dialog = QDialog(self)
        dialog.setWindowTitle("➕ Kirim qo'shish")
        dialog.setFixedSize(380, 260)
        dialog.setStyleSheet(DARK_STYLE)
        ...
        def save():
            ...
            # API orqali kirim qo'shish
            data = {
                "amount": amount,
                "note": note or "Qo'shimcha kirim",
                "user_id": self.user_id
            }
            result = api.post("/api/cash-incomes/", data)
            if result:
                dialog.accept()
                self.load_data()
                QMessageBox.information(self, "Muvaffaqiyat", f"✅ {amount:,.0f} so'm kirim qo'shildi!")
    ...
    def load_data(self):
        try:
            ...
            # API dan dashboard ma'lumotlarini olish
            dashboard_data = api.get("/api/reports/dashboard")
            
            if dashboard_data:
                # Kartalarni yangilash
                stats_map = {
                    'today_sales': dashboard_data.get('today_sales', 0),
                    'today_profit': dashboard_data.get('today_profit', 0),
                    'total_products': dashboard_data.get('total_products', 0),
                    'low_stock': dashboard_data.get('low_stock', 0),
                }
                    ...
                    if key in self.cards:
                        ...
                        if value_label:
                            if key == 'total_products':
                                ...
                                value_label.setText(f"{value:,.0f} so'm")
                
                # Balance
                if 'today_balance' in self.cards:
                    card = self.cards['today_balance']
                    value_label = card.findChild(QLabel, "cardValue")
                    if value_label:
                        balance = dashboard_data.get('today_balance', 0)
                        value_label.setText(f"{balance:,.0f} so'm")
            
            # Top products (API dan olish kerak, hozircha mock)
            self.load_top_products(selected_date)
            
        except Exception as e:
            print(f"❌ Dashboard yuklashda xatolik: {e}")
    
    def load_top_products(self, selected_date):
        """Eng ko'p sotilgan mahsulotlar"""
        try:
            # API dan top products olish
            data = api.get(f"/api/reports/top-products?date={selected_date}")
            
            if data and isinstance(data, list):
                self.top_products_table.setRowCount(len(data))
                for i, item in enumerate(data):
                    self.top_products_table.setItem(i, 0, QTableWidgetItem(str(i + 1)))
                    self.top_products_table.setItem(i, 1, QTableWidgetItem(item.get('name', 'Noma\'lum')))
                    self.top_products_table.setItem(i, 2, QTableWidgetItem(f"{item.get('quantity', 0):,.2f}"))
                    self.top_products_table.setItem(i, 3, QTableWidgetItem(f"{item.get('total', 0):,.0f} so'm"))
